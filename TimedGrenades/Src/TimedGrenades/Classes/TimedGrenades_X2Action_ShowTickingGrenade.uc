...
//-----------------------------------------------------------
class TimedGrenades_X2Action_ShowTickingGrenade extends X2Action;

// The path name of the particle system effect to be played.
var string EffectName;
var string GrenadeIcon;
...
simulated state Executing
{
	...
	{
    local MaterialInstanceConstant IconMIC;
    // find these out...
    local Texture2D IconTex;
    ...
    `log("Particle System Loaded");

    PSComponent.SetFloatParameter('SphereRadius', GrenadeRadius / 48.0f);
    if (GrenadeIcon != "") {
      IconTex = Texture2D(DynamicLoadObject(
        Repl(GrenadeIcon, "img:///", ""),
        class'Texture2D'
      ));
      IconMIC = new (PSComponent) class'MaterialInstanceConstant';
      IconMIC.SetParent(
        MaterialInstanceConstant(
          `CONTENT.RequestGameArchetype("FX_GW_DelayedExplosions.MIC_Grenade")
        )
      );
      IconMIC.SetTextureParameterValue('Icon', IconTex);
      PSComponent.SetMaterialParameter('IconMIC', IconMic);
    }
	}
